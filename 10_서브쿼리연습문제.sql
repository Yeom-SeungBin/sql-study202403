/*
문제 1.
- EMPLOYEES 테이블에서 모든 사원들의 평균급여보다 높은 사원들의 데이터를 출력하세요.
  (AVG(컬럼) 사용)
...
/*
문제 2.
- DEPARTMENTS 테이블에서 manager_id가 100인 부서에 속해있는 사람들의 모든 정보를 출력하세요.
*/
SELECT * FROM employees
...
/*
문제 3.
- EMPLOYEES 테이블에서 “Pat”의 manager_id보다 높은 manager_id를 갖는 모든 사원의 데이터를 
  출력하세요
...
/*
문제 4.
- EMPLOYEES 테이블 에서 first_name 기준으로 내림차순 정렬하고,
...
/*
문제 5.
- EMPLOYEES 테이블에서 hire_date 기준으로 오름차순 정렬하고,
  31~40번째 데이터의 행 번호, 사원id, 이름, 전화번호, 입사일을 출력하세요.
*/
SELECT * FROM 
    (
...
조건) 직원아이디, 이름(성, 이름), 부서아이디, 부서명만 출력합니다.
조건) 직원아이디 기준 오름차순 정렬
*/
SELECT
    e.employee_id,
...
조건) 부서아이디, 부서이름, 매니저아이디, 로케이션아이디, 스트릿_어드레스, 포스트 코드, 시티만 출력합니다.
조건) 부서아이디 기준 오름차순 정렬
*/
SELECT 
...
조건) 로케이션 아이디, 주소, 시티, country_id, country_name만 출력합니다
...
/*
문제 12. 
employees 테이블, departments 테이블을 left 조인 hire_date를 오름차순 기준으로 
11-20번째 데이터만 출력합니다.
...
FROM departments d
-- WHERE d.manager_id IS NOT NULL > 이 데이터에는 manager_id가 null인 경우 사원이 존재 X
WHERE EXISTS (
    SELECT 1
    FROM employees e
    WHERE e.department_id = d.department_id
) -- manager_id는 null이지만 사원이 속해있는 경우
ORDER BY total DESC;
...
/*
문제 16
- 문제 15 결과에 대해 DEPARTMENT_ID 기준으로 내림차순 정렬해서 
